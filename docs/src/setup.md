...
##### FILE_STORAGE_HOPE
This backend is used for reading HOPE dataset images. The deduplication API
receives image filenames (paths/keys) and reads the image bytes directly from
this storage. It should be configured as read-only for the service.
    ```
