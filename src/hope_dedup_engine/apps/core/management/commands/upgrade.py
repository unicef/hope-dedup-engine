...
class Command(BaseCommand):
    ...
    def _run_createsuperuser(self, login: str, email: str) -> bool:
        os.environ["DJANGO_SUPERUSER_USERNAME"] = login
        os.environ["DJANGO_SUPERUSER_EMAIL"] = email

        if password := self.admin_password if login == self.admin_email else "":
