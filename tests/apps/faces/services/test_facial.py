...
def sample_data():
    ...
    return {
        "encoding_pairs": [(EncodingFactory(filename="file1.jpg"), EncodingFactory(filename="file2.jpg"))],
        "ignored_pairs": set(),
...
def test_encode_faces_with_pre_encodings(mock_deepface, mock_storage):
    ...
    mock_deepface.represent.return_value = [{"embedding": [2.0], "face_confidence": 0.1}]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"return_value": [{"embedding": [0.0], "face_confidence": 0.0}]}, Image.StatusCode.NO_FACE_DETECTED),
    ],
)
...
def test_dedupe_images_similarity_threshold(mock_deepface, sample_data, verify_return, expected_result):
    ...
    mock_deepface.verify.assert_called_once_with(
        sample_data["encoding_pairs"][0][0].embedding, sample_data["encoding_pairs"][0][1].embedding
    )
...
def test_dedupe_images_with_facial_error(mock_deepface, sample_data):
    ...
    test_data = copy.deepcopy(sample_data)
    test_data["encoding_pairs"][0][0].status_code = Image.StatusCode.NO_FACE_DETECTED.value
    results = dedupe_images(**test_data)
    assert results == []
    mock_deepface.verify.assert_not_called()
