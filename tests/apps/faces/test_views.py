...
def finding_image_storage(mocker: MockerFixture) -> AzureStorage:
    ...
    mocker.patch(
        "hope_dedup_engine.apps.faces.views.ImagesStorageManager",
...
def test_finding_image_requires_staff(client: MagicMock) -> None:
    url = reverse("faces:finding-image", kwargs={"filename": "image.jpg"})
...
def test_finding_image_serves_file(
    ...
    filename = "image.jpg"
    url = reverse("faces:finding-image", kwargs={"filename": filename})
...
def test_finding_image_missing_returns_404(
    ...
    url = reverse("faces:finding-image", kwargs={"filename": "missing.jpg"})
...
def test_finding_preview_requires_staff(client: MagicMock, finding: MagicMock) -> None:
    url = reverse("faces:finding-preview", kwargs={"pk": finding.pk})
...
def test_finding_preview_context(admin_client: MagicMock, finding: MagicMock) -> None:
    finding.first_filename = "first.jpg"
    finding.second_filename = ""
    ...
    assert ctx["status_label"] == finding.status_display
    assert ctx["first_image_url"] == reverse("faces:finding-image", kwargs={"filename": "first.jpg"})
