...
@pytest.mark.parametrize(
    ...
    [
        (
            {("path1", "path2", 0.2), ("path2", "path3", 0.1)},
            0.3,
            (("path1", "path2"), ("path3", "path2")),
        ),
        (
            {("path1", "path2", 0.2), ("path2", "path3", 0.4)},
            0.3,
            (("path1", "path2"),),
        ),
...
def test_duplicate_groups_builder(checked, threshold, expected_groups):
    def sort_nested_tuples(nested_tuples: tuple[tuple[str]]) -> tuple[tuple[str]]:
        sorted_inner = tuple(
            tuple(sorted(inner_tuple)) for inner_tuple in nested_tuples
        )
    ...
    with patch(
        "hope_dedup_engine.apps.faces.utils.duplicate_groups_builder.config",
        mock_config,
    ):
