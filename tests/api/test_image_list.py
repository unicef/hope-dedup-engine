...
def test_can_list_images(api_client: APIClient, deduplication_set: DeduplicationSet, encoding: Encoding) -> None:
    response = api_client.get(reverse(ENCODING_LIST_VIEW, (deduplication_set.pk,)))
...
def test_cannot_list_images_between_systems(
    ...
    response = another_system_api_client.get(reverse(ENCODING_LIST_VIEW, (deduplication_set.pk,)))
